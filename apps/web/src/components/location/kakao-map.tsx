...
import { validateMapApiKey } from '@/lib/api/kakao';
import type {
  KakaoMapProps,
...
const KakaoMap = ({
  ...
  useEffect(() => {
    const loadKakaoMap = () => {
      ...
      try {
        const apiKey = validateMapApiKey();
        ...
        setMapError(error instanceof Error ? error.message : '지도를 로드할 수 없습니다.');
      }
    };
