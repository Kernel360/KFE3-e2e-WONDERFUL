      ...
      <div className="shadow-xs flex h-[54px] w-full min-w-0 items-center justify-between rounded-[33px] border bg-transparent px-4 py-1 text-base text-neutral-400 transition-[color,box-shadow] focus-within:border-neutral-400 focus-within:ring-[2px] focus-within:ring-neutral-400/50 md:text-sm">
        <div className="flex items-center gap-2 [&>svg]:h-5 [&>svg]:w-5">
          ...
          <input
            ...
            value={value}
            {...props}
            className="selection:bg-primary file:text-foreground aria-invalid:ring-danger-700/20 aria-invalid:border-danger-700 selection:text-primary-500 w-[270px] text-black file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-neutral-400 focus:shadow-none focus:outline-none disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50"
          />
        </div>
        <div className="[&>button>svg]:h-5 [&>button>svg]:w-5 [&>svg]:h-5 [&>svg]:w-5">
          {showResetButton && (
            <button
              type="button"
              onClick={onReset}
              className="h-[20px] cursor-pointer transition-opacity hover:opacity-70"
              title="현재가로 초기화"
            >
